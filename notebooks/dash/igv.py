...
insertion_table_hpv16 = pd.read_csv(outputs_path_hpv16 + 'hpv16_to_hg38_table.csv', dtype={'srr': str, 'id': str }).drop(columns=['sequence'])
outputs_path_hpv18 = 'http://159.196.33.135:8080/hpv18_rnaseq/'
insertion_table_hpv18 = pd.read_csv(outputs_path_hpv18 + 'hpv18_to_hg38_table.csv', dtype={'srr': str, 'id': str }).drop(columns=['sequence'])
outputs_path_hiv1 = 'http://159.196.33.135:8080/hiv1_rnaseq/'
insertion_table_hiv1 = pd.read_csv(outputs_path_hiv1 + 'hiv1_to_hg38_table.csv', dtype={'srr': str, 'id': str }).drop(columns=['sequence'])
...
app.layout = html.Div([
    ...
    dcc.Tabs(id='tab-selection', value='tab-1', children=[
        dcc.Tab(label='Table of Available Samples', value='tab-1'),
        dcc.Tab(label='IGV Visualization', value='tab-2'),
    ]),
...
def render_content(tab, srr, id, donor, recipient):
    ...
    if tab == 'tab-1':
        ...
        # Create to-recipient tracks
        alignment_tracks = []
        for crossing in crossings:
            alignment_tracks.append(
                {
                    # Docs: 
                    ...
                    "coverageColor" : "rgb(210, 100, 102)", "displayMode" : "SQUISHED" #, "autoHeight" : "true",
                })
